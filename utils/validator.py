...
class DataValidator:
	...
	def validate_uuid_api_key(uuid, api_key, rdb_conn=md_context.get_rdb_info()):
		"""
		:param uuid: Unique Identifier of the user
		...
		:return response: Empty String if they are valid, else returns correct error message.
		"""
		template = {}
		template['uuid'] = uuid
		template['api_key'] = api_key
		response = ""
		database_service = RDBDataTable("developer_info", connect_info=rdb_conn, key_columns=["form_id"])
	...
	def validate_uuid_form_id(uuid, form_id, rdb_conn=md_context.get_rdb_info()):
		"""
		:param uuid: Unique Identifier of the user
	...
	def fetch_form_response(form_id, response_id="", 
							rdb_conn=md_context.get_rdb_info(), mongodb_conn=md_context.get_mongo_db_info()):
	...
	def validate_request_endpoint(request, form_id, rdb_conn=md_context.get_rdb_info()):
		"""
		...
		template = {}
		template['form_id'] = form_id
		database_service = RDBDataTable("form_endpoint_mapper", connect_info=rdb_conn, key_columns=["form_id"])
